  ...
  navbarPage("NYC Rat Sightings", id = "navbar",
             ...
             tabPanel("About",
                      ...
                      p("A loved one lives in New York City and is extremely afraid of rats. As a fun project, I decided 
                        to visualize rat sightings throughout the years to help them see areas where rats may be more frequent.
...
server <- function(input, output, session) {
  ...
  # Observe changes in filtered map data and update map accordingly
  observe({
    data <- filteredMapData()  # Get filtered data for the map
  ...
  # Filter data for the trend chart
  ...
  output$rat_trend <- renderHighchart({
    # Get filtered data for the trend chart
    trend_data <- reactive({
      rat_sightings %>%
        filter(Year == input$year_slider) %>%
        count(Month) %>%
        complete(Month = 1:12, fill = list(n = 0)) %>%
        mutate(Month = factor(month.abb[Month], levels = month.abb))  # Ensure proper month order
    })
    
    # Render the line chart
    output$rat_trend <- renderHighchart({
      data <- trend_data()  # Get the reactive data
      req(nrow(data) > 0)  # Ensure that there is data to plot
      
      highchart() %>%
        hc_chart(type = 'spline') %>%  # Use 'spline' for a smooth line
  ...
  output$mostActiveMonth <- renderText({
    ...
    if (nrow(active_month) > 0) {
      # Find the numeric value of the month with the most sightings
